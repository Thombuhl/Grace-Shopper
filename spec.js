...
describe('Shopping', () => {
  ...
  describe('Successful Authentication', () => {
    it('Logs the User In', async () => {
      ...
      response = await app.get('/api/sessions').set('authorization', token);
      expect(response.status).to.equal(200);
  ...
  describe('The Shopping Experience', () => {
    it('User can create an order', async () => {
      ...
      response = await app.post('/api/orders').set('authorization', token);
      expect(response.status).to.equal(200);
    });
    ...
    it('The same cart is returned to user', async () => {
      ...
      response = await app.get('/api/orders/cart').set('authorization', token);
    ...
    describe('adding an item to a cart', () => {
      ...
      describe('not the first time', () => {
        it('updates a lineItem', async () => {
          ...
          const updatedLineItem = response.body.lineItems.find(
            (lineItem) => lineItem.productId === foo.id
          );
  ...
  describe('REST API Product', () => {
    ...
    it('Can GET Single Product', async () => {
      const response = await app.get('/api/products/1');
      expect(response.status).to.equal(200);
    });
  });
});
